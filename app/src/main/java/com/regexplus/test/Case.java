...
public class Case {
    ...
    public void test() {
        ...
        for (CaseResult caseResult: tests) {
            ...
            try {
                FileOutputStream fos = new FileOutputStream("C:/debug/dfa.gv");
            ...
            List<IMatch> matches = deterministicAutomaton.match(new StringStream(string)); //;automaton.match(new StringStream(string));

            int length = matches.size() == 0 ? Match.NO_MATCH : matches.get(0).length();

            //if (matches.size() > 0) {
            //    System.out.println(matches.get(0).start() + " " + matches.get(0).length());
            //}
