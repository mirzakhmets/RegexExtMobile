...
public class DeterministicAutomaton {
    ...
    public Map<Integer, DeterministicState> statesIndex = new HashMap<>();
    ...
    public DeterministicAutomaton(Automaton automaton) {
        ...
        while (!detStack.empty()) {
            ...
            for (int i = 0; i < DeterministicState.LETTER_MAX; ++i) {
                ...
                if (s != null && s.visitIndex != DeterministicState.VISIT_INDEX) {
                    detStack.push(s);
            ...
            for (DeterministicState s : this.states) {
                ...
                for (int i = 0; i < DeterministicState.LETTER_MAX; ++i) {
                    if (s.transitions[i] != null) {
                        try {
                            stream.write(("\tnode_" + s.index + " -> node_" + s.transitions[i].index + " [label=\"" + ((char) i) + "\"];\n").getBytes());
                        } catch (Exception e) {
                            e.printStackTrace();
                        }
                    }
