...
public class DeterministicState {
    ...
    public DeterministicState[] transitions = new DeterministicState[LETTER_MAX];
    ...
    public void closure() {
        ...
        while (!stack.empty()) {
            ...
            if (state.getType() == StateType.TAG) {
                ...
                StateTagPair paira = new StateTagPair(((State) state.getOutputEdges().get(0).getFinish()));
                StateTagPair pairb = new StateTagPair(((State) state.getOutputEdges().get(1).getFinish()));
            ...
            for (IEdge edge: state.getOutputEdges()) {
                if (edge.getType() == EdgeType.EMPTY && ((State) edge.getFinish()).getVisitIndex() != NFA_VISIT_INDEX
                    && !this.states.contains((State) edge.getFinish()))
                {
                    ...
                    if (outState.getVisitIndex() != NFA_VISIT_INDEX && prePass) {
    ...
    public DeterministicState step(char symbol) {
        ...
        DeterministicState deterministicState = new DeterministicState(this.automaton);
        ...
        for (State s: this.orderedStates) {
            for (IEdge e: s.getOutputEdges()) {
                if (e.getType() == EdgeType.LETTER && e.accepts(symbol)) {
                    if (((State) e.getFinish()).getVisitIndex() != NFA_VISIT_INDEX && !deterministicState.states.contains((State) e.getFinish())) {
                        ((State) e.getFinish()).setVisitIndex(NFA_VISIT_INDEX);

                        StateTagPair pair = new StateTagPair(((State) e.getFinish()));
